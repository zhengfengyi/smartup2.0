...
contract Exchange is ExchangeConfig, Ecrecovery{
    ...
    event SellCt(address _tokenAddress, address seller, uint256 amount, uint256 getSut);

    constructor(address _sut, address _fee, address _ctImpl, address _proposal,address _coinStore, address _admin)public ExchangeConfig(_sut, _fee,_ctImpl,_proposal, msg.sender, _coinStore, _admin){
        
    }



function buyCt(address _tokenAddress, uint256 _amount, address _buyer, uint256 fee, bytes32 _hash, bytes memory signature)public {
    ...
    require(coinStore.balanceOf(_tokenAddress,_tokenAddress) >= _amount);

    uint256 costSut = _amount.mul(market.exchangeRate()).div(DECIMALS_RATE);

    require(coinStore.balanceOf(SUT,_buyer) >= costSut);

    coinStore.marketInternalTransfer(_tokenAddress,_tokenAddress,_buyer,_amount);

    coinStore.internalTransferFrom(SUT,_buyer,_tokenAddress,costSut);

    coinStore.internalTransferFrom(address(0), _buyer, feeAccount, fee);
    ...
    if (coinStore.balanceOf(_tokenAddress,_tokenAddress) == 0) {

        sellOutCt(_tokenAddress);

        
        // ctImpl.finishFirstPeriod(_tokenAddress);

        // uint256 toProposal = coinStore.balanceOf(SUT,_tokenAddress).mul(market.exchangeRate().sub(market.recycleRate())).div(DECIMALS_RATE);

        // coinStore.marketInternalTransfer(SUT, _tokenAddress, address(proposal), toProposal);

        // //TODO proposal add recive SUT
        // proposal.recivedSut(SUT,_tokenAddress,toProposal);
    ...
    function sellOutCt(address _tokenAddress) private {
...
//    function exchangeBuyCt(address _tokenAddress,uint256 _amount,address _buyer,uint256 fee) private returns(uint256) {
...
//    }
    ...
    function marketAdminCheck(address _ctAddress, address trader) private {
    ctStore ct = ctStore(_ctAddress);
    
    if (coinStore.balanceOf(_ctAddress,trader) >= ct.totalSupply().div(10) && !ct.isAdmin(trader) && ct.adminSize() < 5 && trader != ct.creator()) {
        ctImpl.addAdmin(_ctAddress, trader);
    }else if(ct.isAdmin(trader) && ct.adminSize() >= 0 && coinStore.balanceOf(_ctAddress,trader) < ct.totalSupply().div(10) && trader != ct.creator()) {
...
function recycleCT(address _tokenAddress, address seller, uint256 amount, uint256 timeStamp, uint256 fee, bytes memory sign) public {
    ...
    require(coinStore.balanceOf(_tokenAddress, seller) >= amount);
    require(coinStore.balanceOf(address(0),seller) >= fee);

    bytes32 sellerHash = keccak256(abi.encodePacked(_tokenAddress, seller, amount, timeStamp, fee));

    require(ecrecovery(sellerHash, sign) == seller);

    uint256 acquireSut = amount.mul(market.recycleRate()).div(DECIMALS_RATE);

    coinStore.internalTransferFrom(_tokenAddress, seller, _tokenAddress, amount);

    coinStore.marketInternalTransfer(SUT, _tokenAddress, seller, acquireSut);

    coinStore.internalTransferFrom(address(0), seller, feeAccount, fee);

    if(coinStore.balanceOf(_tokenAddress, seller) == 0){
        ctImpl.removeCtHolder(_tokenAddress, seller);
    }


    emit SellCt(_tokenAddress,seller,amount,acquireSut);

}

function sellCt(address _tokenAddress, address seller, uint256 _amount, uint256 fee, bytes memory signature) public {
    //require(coinStore.balanceOf(_tokenAddress,seller) >= _amount);
    require(admin.onlyAdmin(msg.sender));

    bytes32 signHash = keccak256(abi.encodePacked(_tokenAddress, seller, _amount, fee)); 

    require(ecrecovery(signHash, signature) == seller);
    ...
    coinStore.internalTransferFrom(_tokenAddress, seller, _tokenAddress, _amount);
    ...
    emit SellCt(_tokenAddress,seller,_amount,acquireSut);
    ...
    function trade(uint256[] memory makerValue, address[] memory makerAddress, uint256[5] memory takerValue, address[3] memory takerAddress, bytes32[] memory rs, uint8[] memory v, bytes memory takerSign)public {
    require(admin.onlyAdmin(msg.sender));
    //check taker sign
    ...
    //only SUT
    require(takerAddress[0] ==  SUT || takerAddress[1] == SUT);

    if(takerAddress[0] == SUT){
        //TODO !
        require(!ctStore(takerAddress[1]).isInFirstPeriod() && !ctStore(takerAddress[1]).dissolved());
        require(ctStore(takerAddress[1]).migrationTarget() == address(0));
    }else{
    ...
    require(feeFills[takerHash].add(takerValue[4]) <= takerValue[3]);

    //have enough sut to pay;
    require(coinStore.balanceOf(takerAddress[0], takerAddress[2]) >= takerValue[0].sub(orderFills[takerHash]).mul(takerValue[1]).div(DECIMALS_RATE));
    ...
    for(uint256 i = 0; i < makerValue.length.div(3); i++){
        ...
        //fee exchange
        coinStore.internalTransferFrom(address(0),takerAddress[2],feeAccount,takerValue[4]);

        feeFills[takerHash] = feeFills[takerHash].add(takerValue[4]);
...
function singleTrade(address _source, address _target, address _taker, address _maker, uint256 _targetAmount, uint256 _sourceAmount, bytes32 _takerHash, bytes32 _makerHash) private {
            ...
            coinStore.internalTransferFrom(_target,_maker,_taker,_targetAmount);

            coinStore.internalTransferFrom(_source,_taker,_maker,_sourceAmount);
            ...
            if (_source == SUT) {
                ...
                if(coinStore.balanceOf(_target,_maker) == 0) {
                ...
                if(coinStore.balanceOf(_source,_taker) == 0){
